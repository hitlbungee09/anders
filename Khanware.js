...
async function showSplashScreen() {
    ...
    style.innerHTML = `
        ...
        @keyframes textGlow {
            0%, 100% { text-shadow: 0 0 10px #72ff72, 0 0 20px #72ff72; }
            50% { text-shadow: 0 0 20px #72ff72, 0 0 40px #72ff72; }
        }
    `;
    document.head.appendChild(style);
    Object.assign(splashScreen.style, {
        position: 'fixed', top: '0', left: '0', width: '100%', height: '100%',
        backgroundColor: '#0a0a0f', display: 'flex', alignItems: 'center',
        justifyContent: 'center', zIndex: '9999', opacity: '0',
        transition: 'opacity 0.5s ease-out', userSelect: 'none',
        fontFamily: "MuseoSans, sans-serif", fontSize: '42px',
        animation: 'fadeIn 0.5s forwards'
    });
    splashScreen.innerHTML = '<span style="color:white;">KHANWARE</span><span style="color:#72ff72; animation: textGlow 2s infinite ease-in-out;">.SPACE</span>';
...
function setupMenu() {
    ...
    watermark.addEventListener('mousedown', e => { isDragging = true; offsetX = e.clientX - watermark.getBoundingClientRect().left; offsetY = e.clientY - watermark.getBoundingClientRect().top; watermark.style.transition = 'none'; watermark.style.transform = 'scale(0.95)'; });
    ...
    document.addEventListener('mouseup', () => { if (isDragging) { isDragging = false; watermark.style.transition = 'all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1)'; watermark.style.transform = 'scale(1)'; } });
    ...
    let menuHover = false, watermarkHover = false, hideTimeout;
    watermark.addEventListener('mouseenter', () => { watermarkHover = true; clearTimeout(hideTimeout); showMenu(); playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/3kd01iyj.wav'); });
    watermark.addEventListener('mouseleave', () => { watermarkHover = false; hideTimeout = setTimeout(() => { if (!menuHover) { hideMenu(); playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/rqizlm03.wav'); } }, 100); });
    dropdownMenu.addEventListener('mouseenter', () => { menuHover = true; clearTimeout(hideTimeout); });
    dropdownMenu.addEventListener('mouseleave', () => { menuHover = false; hideTimeout = setTimeout(() => { if (!watermarkHover) { hideMenu(); playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e4-3032a64a9f32/rqizlm03.wav'); } }, 100); });
}
...
(async () => {
    ...
    try {
        const response = await fetch("https://es.khanacademy.org/api/internal/graphql/getFullUserProfile", {
            method: "POST",
        ...
        console.error("Khanware: Failed to fetch user profile.", error);
    }
    
    sendToast("✅ Senseware v4 Injected Successfully");
    ...
    if (device.apple) {
        await delay(500);
        sendToast(`🍏 Detected Apple device... good luck`);
    }
